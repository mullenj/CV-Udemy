...
def detect(greyimage, image):
    ...
    for (x, y, w, h) in faces:
        ...
        eyes = eye.detectMultiScale(faceRegionGrey, 1.4, 10)
        smiled = smile.detectMultiScale(faceRegionGrey, 2.1, 25)
        for (ex, ey, ew, eh) in eyes:
            cv2.rectangle(faceRegion, (ex,ey), (ex+ew, ey+eh), (0,255,0), 2)
        for (sx, sy, sw, sh) in smiled:
            cv2.rectangle(faceRegion, (sx,sy), (sx+sw, sy+sh), (0,0,255), 2)
    return image
